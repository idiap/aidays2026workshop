# SPDX-FileCopyrightText: Copyright © 2026 Idiap Research Institute <contact@idiap.ch>
# SPDX-FileContributor: William Droz <william.droz@idiap.ch>
# SPDX-License-Identifier: GPL-3.0-only

"""
Data analysis with CodeAgent
"""

from pathlib import Path
from dotenv import load_dotenv
from loguru import logger
from smolagents import OpenAIModel, CodeAgent  # noqa: F401
import os

load_dotenv()


def build_agent():
    if (base_url := os.getenv("LLM_BASE_URL")) is None:
        logger.warning(
            "LLM_BASE_URL environment variable is not set, defaulting to https://api.openai.com/v1"
        )
        base_url = "https://api.openai.com/v1"
    if (api_key := os.getenv("LLM_API_KEY")) is None:
        raise ValueError("LLM_API_KEY environment variable is not set")
    if (model_name := os.getenv("LLM_MODEL_NAME")) is None:
        model_name = "gpt-5.2"
        logger.warning(
            f"LLM_MODEL_NAME environment variable is not set, defaulting to {model_name}"
        )
    model = OpenAIModel(  # noqa: F841
        model_id=model_name,
        api_base=base_url,
        api_key=api_key,
    )
    # TODO
    # https://smolagents.org/docs/agents-guided-tour/#1-toc-title
    # pandas and polars are already installed
    #  agent = ...
    #  return agent


if __name__ == "__main__":
    agent = build_agent()
    user_question = input("Ask the agent: ")
    dataset_path = Path(__file__).parent.parent.parent / "dataset" / "voting.csv"
    # TODO
    # create a prompt that gives the user question and the dataset path, and asks the agent to use pandas or polars to answer the question
    # prompt = ...
    # result = ...
    # print(result)
